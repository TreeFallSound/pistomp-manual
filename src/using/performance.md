...
Raise the limit in [FIXME:where?] and reboot. Each additional port costs about 33 KB of RAM that JACK locks and never releases, so raise it only as far as you need:
...
See [Configuration Guide]({{ '/developers/configuration/#jack-ports-and-memory' | url }}) for how the allocation works.
